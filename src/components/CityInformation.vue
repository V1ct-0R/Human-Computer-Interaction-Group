    ...
    >
          ...
          <span v-else>
            <el-row :gutter="20">
              <el-col :span="16">
                <div>今日空气质量：</div>
                <el-table
                  :data="currentAirQualityTableData"
                  stripe
                  style="width: 100%"
                >
                  <el-table-column
              ...
              <el-col :span="8">
                <div>未来空气质量：</div>
                <el-table
                  :data="futureAirQualityTableData"
                  stripe
                  style="width: 100%"
                >
                  <el-table-column
          ...
          <span v-else>
            <el-row :gutter="20">
              ...
              <el-col :span="12">
                ...
                <div
                  v-html="'今日出行建议：' + weatherTipsToday"
                  style="font-size: 12px"
                ></div>
                <div
                  v-html="'明日出行建议：' + weatherTipsTommorow"
                  style="font-size: 12px"
                ></div>
                <el-divider content-position="right"
                  >来自：在线天气查询接口</el-divider
                >
              </el-col>
            </el-row>
          </span>
      ...
      <el-tab-pane label="其他信息" name="fifth">
        <el-card shadow="always">
          ...
          <span v-else>
            <div
              v-html="'外文名：' + cityForeignName"
              style="margin-top: 10px"
            ></div>
            <div
              v-html="'别名：' + cityOtherName"
              style="margin-top: 10px"
            ></div>
            <div
              v-html="'行政区划代码：' + cityDivisionCode"
              style="margin-top: 10px"
            ></div>
            <div
              v-html="'行政区类别：' + cityType"
              style="margin-top: 10px"
            ></div>
            <div
              v-html="'电话区号：' + cityPhone"
              style="margin-top: 10px"
            ></div>
            <div
              v-html="'邮政编码：' + cityPostcode"
              style="margin-top: 10px"
            ></div>
            <div v-html="'地理位置：' + cityPos" style="margin-top: 10px"></div>
            <div
              v-html="'气候条件：' + cityClimate"
              style="margin-top: 10px"
            ></div>
            <div
              v-html="'著名景点：' + citySpot"
              style="margin-top: 10px"
            ></div>
...
export default {
  ...
  mounted() {
    ...
    getIpAddress()
      .then((response) => {
        ...
        let resultcity = response.data["city"].slice(
          0,
          response.data["city"].length - 1
        );
  ...
  methods: {
    ...
    getInfoFromBaike() {
      ...
      //从维基百科获取信息
      getInfoFromWiki(param)
        .then((response) => {
          console.log(response);
          this.generalInfoWiki =
            response.data[0]["detailedDescription"]["articleBody"];
        })
        .catch((error) => {
          console.log(error);
        });
      ...
      getCurrentAirQuality(airparam)
        .then((response) => {
          ...
          this.currentAirQualityTableData.push({
            ...
            o3: response.now["o3"],
            primary:
              response.now["primary"] == "NA" ? "无" : response.now["primary"],
          });
          for (var i = 0; i < 5; i++) {
            this.currentAirQualityTableData.push({
              ...
              primary:
                response.station[i]["primary"] == "NA"
                  ? "无"
                  : response.station[i]["primary"],
            });
          }
        })
        .catch((error) => {
        ...
        .then((response) => {
          ...
          for (var i = 0; i < response.daily.length; i++) {
            this.futureAirQualityTableData.push({
              ...
              primary:
                response.daily[i]["primary"] == "NA"
                  ? "无"
                  : response.daily[i]["primary"],
            });
          }
      ...
      loadBMap("Ip8dveGyiHj0OyPdyZ1pPm72kG05aF3H")
        .then(() => {
          ...
          this.myMap.centerAndZoom(
            new BMap.Point(this.cityLng, this.cityLat),
            11
          ); // 初始化地图,设置中心点坐标和地图级别
          //添加地图类型控件
    ...
    querySearch(queryString, cb) {
      ...
      var results = queryString
        ? strlist.filter(this.createFilter(queryString))
        : strlist;
    ...
    createFilter(queryString) {
      return (strlist) => {
        return (
          strlist.value.toLowerCase().indexOf(queryString.toLowerCase()) === 0
        );
      };
    },
    //切换标签页
